...
- [x] Novo filtro "Estoque Atual"
	- [x] Multisseleção {Todos, Positivo, Zerado, Negativo}
...
- Não existem dados na base de teste
	- Consegui inserindo pelo banco de dados no postgres, mas no oracle não deu certo, vou verificar com o time
...
## Resoluções
- Criamos um novo componente para seleção múltipla para não dar problema com os reports que já existem
- Passamos ?multiple=true para a tela JSP que lista os fornecedores
- Para o relatório, criamos no .js um objeto com divisaoKeys e fornecedorKeys com as keys em lista

## Erros
- [[Gledson]] apontou erro no filtro de estoque
- Coluna Custo Bruto está vindo em branco